...
##########################
pdf("../figures/negbinom_fig3.pdf", width = 12, height = 8)
...
band(tObs, 
     ...
     col = "gray65")
lines(tObs, apply(df, 2, mean), lwd=2)
lines(c(0,168), c(0,0), lty=2)
title("Change in hospital admissions", font.main=1)

plot(tObs, tdi * 100, ylim=c(0,100), type="l",
     xaxt="n", xlab="Number of days since March 1th 2020",
     ylab="Trend Direction Index [%]", lwd=2, xlim=c(0,168))
axis(1, seq(0, 168, 14), cex.axis=0.90)
lines(c(0,168), c(50,50), lty=2)
text(max(tObs), round(tdi[length(tObs)]*100, 2), 
     paste(round(tdi[length(tObs)]*100, 2), "%", sep=""), pos=3, cex=0.7)
