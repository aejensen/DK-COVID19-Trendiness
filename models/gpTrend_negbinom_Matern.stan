...
parameters {
  ...
  real<lower = 0> alpha;  //prior GP variance
  real<lower = 0> rho;    //prior GP length scale
  real<lower = 1> nu; //prior GP Matern order (nu > k => k times MS differentiable)
...
transformed parameters{
  ...
  real k2 = lgamma(0.5 + nu) - lgamma(nu);
  
  for(p in 1:P){ 
    //diagSPD[p] = sqrt(spd_Matern(alpha, rho, nu, sqrt(lambda(L, p)))); 
    real k3 = (-0.5-nu)*log((2*nu)/(square(rho)) + 4*square(pi() * sqrt(lambda(L, p))));
    diagSPD[p] = sqrt(exp(k1 + k2 + k3));
  }
